...
def calculate_averageTopicScores(mastermatrixfile, targets, outfolder):
    ...
    with open(mastermatrixfile, "r") as infile:
        ...
        ## Optional, for taking specific topics out manually
        #mastermatrix = mastermatrix.drop("205", axis=1)
        ## Optional, for taking specific groups of texts out manually
        mastermatrix = mastermatrix[mastermatrix["subgenre"].isin(["policier"])] # = only subgenre policier
...
def make_wordle_from_mallet(word_weights_file, 
    ...
    for topic in range(0,numOfTopics):
        ...
        plt.imshow(wordcloud.recolor(color_func=get_color_scale, random_state=None))
...
def get_heatmap_dataToPlot(average, mode, sorting, firstWordsFile, topTopicsShown, 
    ...
    with open(average, "r") as infile:
        ...
        ## Sort and discard lowest n entries
        allScores = allScores
        #print(allScores.head())
        ## Optionally, limit display to part of the columns 
        allScores = allScores.drop("other", axis=0)
        allScores = allScores.drop("n.av.", axis=0)
        allScores = allScores.drop("fantastic", axis=0)
        allScores = allScores.drop("fantastique", axis=0)
        allScores = allScores.drop("archaique", axis=0)    
        allScores = allScores.drop("blanche", axis=0)    
        allScores = allScores.drop("sciencefiction", axis=0)    
        allScores = allScores.drop("nouveau-roman", axis=0)    
        allScores = allScores.drop("litteraire", axis=0)
        allScores = allScores.drop("suspense", axis=0)
        allScores = allScores.drop("neopolar", axis=0)
        allScores = allScores.drop("thriller", axis=0)
        allScores = allScores.drop("espionnage", axis=0)
        allScores = allScores.drop("criminel", axis=0)
        allScores = allScores.drop("social", axis=0)
        #allScores = allScores.drop("133", axis=1)
        #allScores = allScores.drop("145", axis=1)
        #allScores = allScores.drop("49", axis=1)
        #allScores = allScores.drop("5", axis=1)
        #allScores = allScores.drop("113", axis=1)
        #allScores = allScores.drop("91", axis=1)
        #allScores = allScores.drop("83", axis=1)
        #allScores = allScores.drop("174", axis=1)
        #allScores = allScores.drop("134", axis=1)
        #allScores = allScores.drop("54", axis=1)
        #allScores = allScores.drop("99", axis=1)
        #allScores = allScores.drop("118", axis=1)
        #allScores = allScores.drop("206", axis=1)
        #allScores = allScores.drop("182", axis=1)
        #allScores = allScores.drop("11", axis=1)
        #allScores = allScores.drop("215", axis=1)
        #allScores = allScores.drop("84", axis=1)
        #allScores = allScores.drop("97", axis=1)
        #allScores = allScores.drop("93", axis=1)
        #allScores = allScores.drop("144", axis=1)
        #allScores = allScores.drop("112", axis=1)
        #allScores = allScores.drop("203", axis=1)
        #allScores = allScores.drop("7", axis=1)
        #allScores = allScores.drop("78", axis=1)
        #print(allScores.head())
        ## Several normalisation strategies
        colmeans = allScores.mean(axis=0) 
        #print(colmeans)
        rowmeans = allScores.mean(axis=1, numeric_only=True)
        print(rowmeans)
        allstd = allScores.stack().std() #std for entire df
        if mode == "experimental": # for trying things out
            allScores = (allScores - colmeans) ** 0.6
        if mode == "divbymean": # mean normalization1
            allScores = allScores / colmeans
            #print(allScores.head())
        if mode == "divbyrowmean": # mean normalization1
            allScores = allScores / rowmeans
        elif mode == "minusmean": # mean normalization2
            allScores = (allScores - colmeans) * 10
        elif mode == "zscores": # zscore transformation
        ...
        #allScores = allScores.drop("suspense", axis=1)
        #allScores = allScores.drop("espionnage", axis=1)
        #allScores = allScores.drop("thriller", axis=1)
        #allScores = allScores.drop("litteraire", axis=1)
        #allScores = allScores.drop("neopolar", axis=1)
        #allScores = allScores.drop("archaique", axis=1)
        #allScores = allScores.drop("Amila", axis=1)
        #allScores = allScores.drop("Barbara", axis=1)
        #allScores = allScores.drop("Bon", axis=1)
        #allScores = allScores.drop("Echenoz", axis=1)
        #allScores = allScores.drop("Zola", axis=1)
        #allScores = allScores.drop("Butor", axis=1)
        #allScores = allScores.drop("Modiano", axis=1)
        #allScores = allScores.drop("Izzo", axis=1)
        #allScores = allScores.drop("Manotti", axis=1)
        #allScores = allScores.drop("Japrisot", axis=1)
        #allScores = allScores.drop("Manchette", axis=1)
        #print(allScores)
        ...
        #dataToPlot = dataToPlot.sub(5)
        #dataToPlot = dataToPlot.div(0.2)
        #allScores = allScores.drop("Amila", axis=1)
        #dataToPlot = dataToPlot.iloc[:,0:16]
        #print(dataToPlot)
        return dataToPlot
...
def create_distinctiveness_heatmap(dataToPlot, 
    ...
    sns.heatmap(dataToPlot, annot=True, cmap="BrBG", square=False)
    # Nice: bone_r, copper_r, PuBu, OrRd, GnBu, BuGn, YlOrRd
    # Bipolar: coolwarm, BrBG, RdBu_r
    ...
    elif mode == "minusmean" and sorting != "std":         
...
def build_itemScoreMatrix(averageDatasets, targetCategory, 
    ...
    for averageFile in glob.glob(averageDatasets): 
        if targetCategory in averageFile:
            ...
            #itemScoreMatrix = itemScores.iloc[0:topicsPerItem,0:-1]  ## STANDARD
            itemScoreMatrix = itemScores.iloc[60:-60,0:-1]
            ...
            #itemScoreMatrix = itemScoreMatrix.drop("Vargas", axis=0)
            #itemScoreMatrix = itemScoreMatrix.drop("Ponson", axis=0)
...
def perform_itemClustering(itemScoreMatrix, targetCategory, method, metric, 
    ...
    plt.setp(plt.xticks()[1], rotation=90, fontsize = 8)
